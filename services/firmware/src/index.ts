/**
 * Selene firmware service (port 3004)
...
const port = Number(process.env.FIRMWARE_PORT ?? SERVICE_PORTS.firmware);
...
app.get("/api/firmware/status", async () => ({
  service: "firmware",
  ready: false,
...
console.log(`[firmware] scaffold listening on :${port}`);
